...
export const setRefreshToken = (headers: Headers) => {
   ...
   // кука должна сразу прилетать на it-gulnaz.xxx чтобы nextjs мог тоже достать рефреш-токен, так как акссес он не
   ...
   headers.append(
      'Set-Cookie',
      `refresh-token=xxx${index}; Domain=.it-gulnaz.xxx; Path=/; Secure; HttpOnly; SameSite=Strict; Max-Age=300000;`,
   );
};
